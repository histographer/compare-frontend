      ...
      >
        <template #default="{row: member}">
            ...
            <span class="icons">
              <a @click="confirmToggleManager(member)">
                <i
                  ...
                  :class="{ disabled: member.role === contributorRole.value }"
                ></i>
              </a>
              <a v-if="member.role !== contributorRole.value" @click="toggleRepresentative(member)">
                <i
                  class="fas fa-flag"
                  :class="{ disabled: member.role !== representativeRole.value }"
                ></i>
              </a>
            </span>
          </b-table-column>
          ...
          <b-table-column label="" centered width="50">
            <router-link
              :to="`/project/${project.id}/activity/user/${member.id}`"
  ...
  methods: {
    ...
    async toggleManager(member) {
      try {
        ...
        this.$notify({
          type: 'error',
          text: this.$t('notif-error-change-role', { username: fullName(member) })
        });
      }
    },
