...
import ComparisonImage from '../viewer/ComparisonImage.vue';
import ImageSelector from '../viewer/ImageSelector';
  ...
  methods: {
    findIdViewer() {
      ...
      for (let id in this.viewers) {
        ...
        let imagesViewer = Object.values(this.viewers[id].images)
    ...
    async fetchPage() {
      ...
      let images = await this.imageCollection.fetchAll();
      this.images = images.array;
      let cells = [];
      // Get the first two images from the project currently
      for (let i = 0; i < 2; i++) {
        let index = String(i);
